        ...
        switch (os) {
            .darwin => {
                if (std.mem.indexOf(u8, name_s, "$")) |suffic_index| {
                    ...
                    if (hook_table.get(name_s)) |alt_entry| break :alt alt_entry;
                }
            },
            .windows => {
                if (std.mem.startsWith(u8, name_s, "_")) {
                    name_s = name_s[1..];
                    if (hook_table.get(name_s)) |alt_entry| break :alt alt_entry;
                    if (std.mem.endsWith(u8, name_s, "32")) {
                        name_s.len -= 2;
                        if (hook_table.get(name_s)) |alt_entry| break :alt alt_entry;
                    }
                } else if (std.mem.endsWith(u8, name_s, "A")) {
                    name_s.len -= 1;
                    if (hook_table.get(name_s)) |alt_entry| break :alt alt_entry;
                }
            },
